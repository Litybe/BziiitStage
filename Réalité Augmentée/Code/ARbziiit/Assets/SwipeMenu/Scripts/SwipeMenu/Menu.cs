...
namespace SwipeMenu
{
	...
	public class Menu : MonoBehaviour
	{
		...
		{
			UpdateMenuItemsPositionInWorldSpace ();
            //this.transform.LookAt(cameraGO.transform);
		...
		private void UpdateMenuItemsPositionInWorldSpace ()
		{
			for (int i = 0; i < menuItems.Length; i++) {
                ...
                //Debug.Log(i + " " + CalculateMenuItemRotation(offsetx));
                Vector3 pos = new Vector3 (posx, 0f, posz);
				menuItems [i].transform.position = pos;
				Vector3 euler = new Vector3 (0, CalculateMenuItemRotation (offsetx), 180);
